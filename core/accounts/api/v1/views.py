...
class CustomObtainAuthToken(ObtainAuthToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({"token": token.key, "user.id": user.pk, "email": user.email})
...
class ChangePasswordApiView(generics.GenericAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            self.object.save()
            return Response(
                {"datatil": "password change successfully"}, status=status.HTTP_200_OK
            )
...
class ActivationApiView(APIView):
    def get(self, request, token, *args, **kwargs):
        try:
            ...
            return Response(
                {"datail": "token is not valid"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        user_obj.save()
        return Response(
            {"details": "yout account have been verified and avtivated successfullt"}
        )
...
class ActiveResendApiView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        EmailThread(email_obj).start()
        return Response(
            {"detail": "user activation resend succefully"}, status=status.HTTP_200_OK
        )
